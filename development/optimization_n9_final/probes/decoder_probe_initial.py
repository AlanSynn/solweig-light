...
def run(out_path):
    ...
    for bad in (0,1,7,8,31):
        ...
        for fn in (generic,literal_modes):
    ...
    for name in ('binary','mixed','raw'):
        for n in (128,1024):
            ...
            for fn in (generic,literal_modes):fn(payloads,modes,0,n,p,out,8)
            raw={'generic':[],'literal_modes':[]}
            for rep in range(9):
                seq=(('generic',generic),('literal_modes',literal_modes))
                if rep%2:seq=seq[::-1]
                for label,fn in seq:
            ...
            timings.append({'mode_mix':name,'pixels':n,'patches':p,'width':8,
                ...
                'raw_ms':raw})
    assembly={}
    for label,fn in (('generic',generic),('literal_modes',literal_modes)):
    ...
    for t in timings:print(t['mode_mix'],t['pixels'],round(t['generic_median_ms'],6),round(t['literal_median_ms'],6),round(t['ratio_generic_over_literal'],3))
