...
## Claude Code worktree base verification

A native `isolation: worktree` may start from the repository default branch rather than the in-progress parent HEAD, depending on installed settings/version. Set the supported `worktree.baseRef: head` explicitly or create the worktree with an immutable Git SHA; then verify the actual merge-base/source before edits. [C08]

Each worktree also needs the current nonsecret instructions and test environment explicitly available. Do not copy `.env`, tokens or provider settings to make a task convenient. Use named authorized profiles outside the repository and read-only shared fixtures with correct identities.
